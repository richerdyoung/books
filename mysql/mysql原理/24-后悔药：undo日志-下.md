...
![image_1d79gnaudfg2psk2l7mi1146r20.png-104.8kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fcc0ca928?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![image_1d79o8ra61jh6k0k6af13vpg329.png-103.9kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fcc9ccd4f?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![image_1d79nq0ge1gn7pmcaa7gma1uh62q.png-71kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1067" height="377"></svg>)

> 小贴士： 上述链表结构我们在前边的文章中频频提到，尤其是在表空间那一章重点描述过，不过我不敢奢求大家都记住了，所以在这里又强调一遍，希望大家不要嫌我烦，我只是怕大家忘了学习后续内容吃力而已～

FIL\_PAGE\_UNDO_LOG页面
---------------------
...
![image_1d79ec33apm47brq901sur3bi16.png-63.2kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="938" height="589"></svg>)
...
![image_1d79ohe8u1uqgh1e978nji1ip2m.png-71.8kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="560" height="431"></svg>)
...
*   `TRX_UNDO_PAGE_TYPE`：本页面准备存储什么种类的`undo日志`。
    ...
    *   `TRX_UNDO_UPDATE`（使用十进制`2`表示），除了类型为`TRX_UNDO_INSERT_REC`的`undo日志`，其他类型的`undo日志`都属于这个大类，比如我们前边说的`TRX_UNDO_DEL_MARK_REC`、`TRX_UNDO_UPD_EXIST_REC`啥的，一般由`DELETE`、`UPDATE`语句产生的`undo日志`属于这个大类。
    ...
    > 小贴士： 之所以把undo日志分成两个大类，是因为类型为TRX\_UNDO\_INSERT_REC的undo日志在事务提交后可以直接删除掉，而其他类型的undo日志还需要为所谓的MVCC服务，不能直接删除掉，对它们的处理需要区别对待。当然，如果你看这段话迷迷糊糊的话，那就不需要再看一遍了，现在只需要知道undo日志分为2个大类就好了，更详细的东西我们后边会仔细唠叨的。
...
*   `TRX_UNDO_PAGE_FREE`：与上边的`TRX_UNDO_PAGE_START`对应，表示当前页面中存储的最后一条`undo`日志结束时的偏移量，或者说从这个位置开始，可以继续写入新的`undo日志`。
    ...
    ![image_1d79s1ib21rku5fkq3to1e14313.png-54kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="810" height="260"></svg>)
...
![image_1d79v7bib12041n9d1gpe1t8a10jc1g.png-60.8kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1172" height="381"></svg>)
...
![image_1d7a6t4va1e0c1l9t1hq51vlt183l2d.png-54.6kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1173" height="404"></svg>)

另外，设计`InnoDB`的大叔规定对普通表和临时表的记录改动时产生的`undo日志`要分别记录（我们稍后阐释为啥这么做），所以在一个事务中最多有4个以`Undo页面`为节点组成的链表：

![image_1d7bg5o7c3t11nch988lj51hsl9.png-106.8kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1107" height="633"></svg>)
...
![image_1d7blo9dp1m4tj1f1ke9te11654m.png-96.8kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1145" height="573"></svg>)
...
![image_1d7bp5ndt171bb6rkohot41e023.png-75.7kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="694" height="361"></svg>)
...
![image_1d7brcccqah1rdn10573vh1onip.png-70.5kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="915" height="555"></svg>)

可以看到这个`Undo`链表的第一个页面比普通页面多了个`Undo Log Segment Header`，我们来看一下它的结构：

![image_1d7bsbk8o15ja2bk1s9p11c51vs2p.png-80.2kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="642" height="555"></svg>)
...
![image_1d7cbktqb16oqih12mqmghn2a1m.png-82kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="964" height="605"></svg>)

这个`Undo Log Header`具体的结构如下：

![image_1d7cfr3cjsec64714qgucc1s8s9.png-122.8kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="500" height="633"></svg>)
...
![image_1d7cocjrk1dvm1ehg16da1di4mfb16.png-87kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1134" height="502"></svg>)
...
*   insert undo链表
    ...
    ![image_1d7el7sg0pje14rjc06rp51r8r1m.png-91.7kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="857" height="514"></svg>)
...
*   update undo链表
    ...
    ![image_1d7elkmtjdbv1kth1c9jk17is123.png-126.3kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1003" height="560"></svg>)
...
![image_1d7gs2k8u1i9i15r71dg97fj10jv9.png-88kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="676" height="541"></svg>)
...
*   `TRX_RSEG_MAX_SIZE`：本`Rollback Segment`中管理的所有`Undo页面`链表中的`Undo页面`数量之和的最大值。换句话说，本`Rollback Segment`中所有`Undo页面`链表中的`Undo页面`数量之和不能超过`TRX_RSEG_MAX_SIZE`代表的值。
    ...
    > 小贴士： 无限大其实也只是个夸张的说法，4个字节能表示最大的数也就是0xFFFFFFFF，但是我们之后会看到，0xFFFFFFFF这个数有特殊用途，所以实际上TRX\_RSEG\_MAX_SIZE的值为0xFFFFFFFE。
...
![image_1d7214jha1cua1dgu1r6718091gbfm.png-27.4kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="665" height="465"></svg>)

每个8字节的格子的构造就像这样：

![image_1d721oblche9a4pmng15p1dk313.png-9.9kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="362" height="161"></svg>)
...
![image_1d7h72gvlin31far16h11elj16tu1m.png-97kB](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1070" height="618"></svg>)
